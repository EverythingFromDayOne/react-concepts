...
**Styling is orthogonal to the Compiler and to a11y.** The [React Compiler](../rendering/react-compiler-deep-dive.md) optimizes re-renders and has nothing to do with your styling choice. And utility classes or scoped modules don't make markup *accessible* — roles, labels, and focus order are a separate concern ([`accessibility-in-react`](./accessibility-in-react.md), *planned*). A pretty button with no accessible name is still broken.
